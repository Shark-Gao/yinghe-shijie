...
- 不得重复推荐同一个源视频；同一武器类别、同一系列或相近选题可以重复，只要源视频 URL 不同。
- 永久排除频道 `3Blue1Brown`：不得推荐其任何视频；即使用户指定，也先说明该频道已列入禁推名单，再改选其他来源。
- 选题要适合系列化，不要随机跳题。
- 军工装备是高优先级；候选质量相当时，优先航母、潜艇、战机、导弹防御、无人装备等少真人、强工程可视化内容。
- 用户要求每日三条时，男性向三条可以重复武器类别、作战环节或系列；不同类别只在候选质量接近时作为平局排序，不得因方向重复直接淘汰。无论是否同方向，同一个源视频仍不得重复。
...
3. 严格选择用户要求的数量；没有指定数量时选 1 条。每天恰好推荐 3 条时，男性向可优先覆盖不同武器或作战环节，但这只是候选质量接近时的排序项，不是方向去重门槛；配置为 6 条的自动任务，则男性向 3 条和女性向 3 条分别执行。
...
> 本条只适用于“推荐视频/选片记录”流程。源片一旦进入 `yinghe-short-video` 或 `yinghe-long-video` 制作流程，封面按对应制作技能的默认规则执行：默认生成 `16:9`、`4:3`、`3:4` 三种封面；只有用户明确要求跳过时才省略。
...
- 三条男性向日更允许重复武器类别、作战环节或系列；只有在候选质量相当时，才按 `#军工装备`、`#导弹`、`#战斗机`、`#航母`、`#潜艇`、`#防空反导`、`#无人机`、`#雷达`、`#电子战`、`#军事纪录片` 的顺序打破平局；精确源视频 URL 查重仍是唯一硬性去重条件；

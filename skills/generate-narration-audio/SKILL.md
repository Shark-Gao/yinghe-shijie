...
description: 当用户要求为硬核视界生成源片等长中文解说、CosyVoice 或 Edge 中文配音、自动压低原声的混音，或根据 SRT/VTT/TXT 生成视频文字注释 JSON 时使用。男性向改写只围绕军事武器、武器系统、作战机制及直接相关军工使用场景，遵循普通观众问题、先给答案、事实边界和多平台包装规则。
---

# 生成中文解说音频

把双语、英文或中英字幕转换为硬核视界的源片等长中文解说。内部可以生成时间线、双语校验字幕、备用音频和画面注释；默认用户只取一个无 BGM 的中文解说 MP4 和一个句子级中文 SRT，不把备用资产混在正式取件文件中。

只使用“等长全程”模式，不制作短混剪或断续的提示音轨。

默认风格是沉稳、清晰、纪录片式的中文口播，重点解释武器系统、作战机制和能力边界；不得写成营销文案，也不能逐句直译字幕。
...
不得从泛泛背景或源频道介绍开始。涉及地图、战争背景或国际局势时，要分开事实、来源观点和推断，保留不确定性，不能把评论写成事实。武器系统内容的重要解释必须能在画面、图示、地图或字幕中找到依据。
...
11. 用 `scripts/timeline_to_srt.py --chinese-only` 根据实际配音时间线导出纯中文 SRT。内部如需事实校验可以另行保留双语 SRT，但默认不作为用户取件文件。随后用 `scripts/split_srt_cues.py --sentence-only --max-chars 20` 按完整句子拆分；长句只换行，不把一个句子硬切成多个字幕条目。不要覆盖原始字幕。
...
18. 默认最终只报告无 BGM 的中文解说 MP4 和句子级中文 SRT；只有用户明确索要工程资产时，才报告时间线 JSON、注释 JSON、备用音频、BGM 混音和自动混音。
...
- 涉及地图、战争背景或国际局势时，避免对国家、战争或联盟下绝对结论，除非有当前核验和源片依据；优先解释武器、装备、后勤、部署、探测、制导、拦截或突防等具体机制。
...
始终从实际配音时间线导出独立的中文 SRT。内部可以保留中英双语校验版，但用户默认取件的字幕必须只含中文，不是原始双语字幕的替代品；字幕显示层按完整句子拆分，不要求一个口播段只显示一条字幕。

- 使用改写后的中文 `text` 和实际配音时间码；
- 用户交付 SRT 只写中文；一个口播段可以对应多条字幕段；
- 默认按句号、问号、感叹号和分号拆分，一句对应一条字幕；逗号只作为句内停顿，不默认拆成独立字幕；
- 使用 `scripts/split_srt_cues.py --sentence-only --max-chars 20` 生成剪映版，`20` 只控制显示换行，不改变句子条目；不要为了拆分改变 MP3 或口播时间线。
- 字幕显示文案默认去掉每条字幕末尾的句号、逗号、问号、感叹号、分号和冒号；句内标点可以保留，用于阅读停顿和理解。
...
| 改写解说语速 | `-8%` |
| 输出目录 | `outputs/audio/` |
| 中文解说字幕 | 由实际配音时间线生成纯中文、句子级 `*_中文字幕.srt` |
...
- 中文字幕直读加 BGM MP3；
- 生成了自动混音时，提供自动混音 MP3；
- 中文解说 SRT 已按完整句子拆分，长句只做一到两行显示，并确认没有大段字幕、单字闪现或字幕末尾标点；
- 已核验的时长、声音和语速。

同时说明：默认只交付无 BGM 的中文解说 MP4 和句子级中文 SRT；直读、人声、BGM、双语字幕和 JSON 均属于内部备用资产，除非用户另行索取。

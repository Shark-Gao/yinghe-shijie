...
11. 用 `scripts/timeline_to_srt.py` 根据改写后的解说时间线导出 `*_中文解说.srt`。每个字幕段包含中文解说和下一行英文翻译，并使用对应解说时间码。导出后，如单条中文过长，再用 `scripts/split_srt_cues.py` 按自然停顿拆成多段时间显示；默认每条字幕最多 `20` 个汉字，优先保持一到两行可读。不要覆盖原始双语字幕。
...
始终从改写后的 `*_等长解说时间线.json` 导出独立的 `*_中文解说.srt`。它是供中文解说音频使用的中英双语字幕轨，不是原始双语字幕的替代品。字幕显示层可以再按自然停顿拆分，不要求一个口播段只显示一条字幕。

- 使用改写后的中文 `text`、对应 `english_text` 和时间线时间码；
- 每个 SRT 段先写中文，再写英文翻译；一个口播段可以对应多条字幕段；
- 中文单条字幕默认不超过 `20` 个汉字，优先在句号、分号、冒号和逗号处拆分；拆分后的时间按原口播段字数比例分配，首尾仍与原时间范围对齐；
- 中文解说只显示中文时，使用 `scripts/split_srt_cues.py --max-chars 20` 生成更适合剪映的分段 SRT；不要为了拆分改变 MP3 或口播时间线。
...
- 中文字幕直读加 BGM MP3；
- 生成了自动混音时，提供自动混音 MP3；
- 中文解说 SRT 已按自然停顿拆分，单条目标约 `20` 个汉字、最长不超过 `26` 个汉字，并确认没有三行长句、单字闪现或字幕末尾标点；
- 已核验的时长、声音和语速。
